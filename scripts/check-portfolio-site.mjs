...
const destinationTransitionFrames = (await fs.readdir("public/assets/destination-transition")).filter(file => /^frame_\d{5}\.png$/.test(file));
assert.equal(destinationTransitionFrames.length, 249);
assert.match(structure, /class=["']destination-animation-frame-preview["']/);
assert.match(structure, /src=["']\.\.\/assets\/destination-transition\/frame_00001\.png["']/);
...
assert.match(structure, /src=["']\.\.\/assets\/directory-transition\/frame_00001\.png["']/);
...
const transitionFrames = (await fs.readdir("public/assets/home-to-directory")).filter(file => /^frame_\d{6}\.png$/.test(file));
assert.equal(transitionFrames.length, 143);
assert.match(homepage, /class=["']home-transition-frame-preview["']/);
assert.match(homepage, /src=["']\.\/assets\/home-to-directory\/frame_000001\.png["']/);
